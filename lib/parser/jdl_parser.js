...
const _ = require('lodash'),
  ...
  JDLMicroserviceOption = require('../core/jdl_microservice_option'),
...
function addOption(key, value, microservicePath) {
  let option = null;
  if (microservicePath) {
    option = new JDLMicroserviceOption({
      value: value,
      entityNames: document[BinaryOptions.BINARY_OPTIONS.MICROSERVICE][value].list,
      excludedNames: document[BinaryOptions.BINARY_OPTIONS.MICROSERVICE][value].excluded,
...
function fillBinaryOptions() {
  _.forEach(BinaryOptions.BINARY_OPTIONS, (optionValue) => {
    _.forEach(document[optionValue], (documentOptionValue, documentOptionKey) => {
      if (optionValue === BinaryOptions.BINARY_OPTIONS.MICROSERVICE) {
        addOption(optionValue, documentOptionKey, documentOptionValue.path);
        return;
      }
